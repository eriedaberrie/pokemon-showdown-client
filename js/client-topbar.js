(function ($) {
	...
	this.Topbar = Backbone.View.extend({
		...
		initialize: function () {
			...
			this.$el.html('<img class="logo" src="' + Dex.resourcePrefix + 'pokemonshowdownbeta.png" alt="Pok&eacute;mon Showdown! (beta)" width="146" height="44" /><div class="maintabbarbottom"></div><div class="tabbar maintabbar"><div class="inner"></div></div><div class="userbar"></div>');
	...
	var OptionsPopup = this.OptionsPopup = Popup.extend({
		...
		update: function () {
			...
			buf += '<p title="On by default because Uranium has BW-styled sprites, if you uncheck this you\'re medically insane."><label class="optlabel"><input type="checkbox" name="bwgfx"' + (!Dex.prefs('bwgfx') ? ' checked' : '') + ' /> Use BW sprites instead of XY models</label></p>';
	...
	var CustomBackgroundPopup = this.CustomBackgroundPopup = Popup.extend({
		...
		initialize: function () {
			...
			buf += '<button name="setBg" value="uranium"' + (cur === 'uranium' ? ' class="cur"' : '') + '><span class="bg" style="background-position:0 -' + (90 * 5) + 'px"></span>Uranium</button>';
	...
	this.LoginPasswordPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			if (data.special === '@gmail') {
				...
				buf += '<small>Keep in mind you\'re sending this over an http connection so I wouldn\'t use a very private password here if I were you.</small>';
